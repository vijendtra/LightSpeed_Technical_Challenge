...
class TorrentParser(object):
    ...
    class TorrentStr(object):
        ...
        def parse_str(self):
            ...
            str_len = self.parseNumber(delimiter=self.STR_LEN_VALUE_SEP)

            if not str_len:
                raise ParsingError('Empty string length found while parsing at position %d'
                                    % self.torr_str.pos)
        ...
        def parse_int(self):
            ...
            return self.parseNumber(delimiter=self.INT_END)

        def parseNumber(self, delimiter):
    ...
    def __init__(self, torrent_file_path):
        ...
        with open(torrent_file_path) as torr_file:
            ...
            self.torrent_str = self.TorrentStr(torrent_content)

        self.parsed_content = self.parseTorrent()

    def get_tracker_url(self):
        ''' Returns the tracker URL from the parsed torrent file. '''

        return 'The Tracker URL..=> ' \
            + self.parsed_content.get('announce')
    ...
    def get_creation_date(self, time_format='iso'):
        ...
        if time_stamp:
            ...
            if time_format == 'iso':
                return 'Creation Date => ' + time_stamp.isoformat()
            else:
                return 'Creation Date => ' + time_stamp
    ...
    def get_client_name(self):
        ...
        return 'Client that created the file => ' \
            + self.parsed_content.get('created by')
        ...
        if files_info:  # 'info' should be present in all torrent files. Nevertheless..
            ...
            if multiple_files_info:  # multiple-file torrent
                for file_info in multiple_files_info:
                    ...
                    parsed_files_info.update({'Name': files_info.get('name'
                            )})
                    parsed_files_info.update({'Length': files_info.get('name'
                            )})
                    parsed_files_info.update({'Checksum': files_info.get('md5sum'
                            )})
            else:
    ...
    def parseTorrent(self):
        ...
        if parsed_char == self.DICT_LIST_END:
            ...
            parsed_dict = {}
            while True:
                dict_key = self.parseTorrent()
                if not dict_key:
                    break  # End of dict
                dict_value = self.parseTorrent()  # parse value
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        test_files_rel_path = '/../Torrent_File'
